/* Fitness Master — one-time Progress cleanup */
(function(){
  'use strict';
  ...
  function restore(){
    ...
    // Version 5 cleanup was already applied: never overwrite later real workouts.
    if(Number(s.progressResetVersion||0)>=5)return false;
    // First migration only: preserve the legitimate Push 2 session as the authoritative history.
    s.logs=canonical;
    s.dataResetVersion=Math.max(Number(s.dataResetVersion||0),5);
    s.progressResetVersion=5;
