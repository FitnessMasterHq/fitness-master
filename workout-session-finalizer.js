...
const x=a[i],now=new Date().toISOString();a[i]=Object.assign({},x,{status:'completed',completedAt:now,updatedAt:now});s.workoutSessions=a;s.lastWorkout=a[i].workout;write(s);window.dispatchEvent(new CustomEvent('fm-cloud-data-updated'));if(typeof window.render==='function')window.render('dashboard');}
