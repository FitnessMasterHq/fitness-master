/* Fitness Master — Firestore sync v1: local-first cloud backup */
(function(){
  'use strict';
  ...
  async function push(){
    ...
    const s=local(), text=sig(s); if(text===last)return; busy=true;
    try{await ref().set({state:s,schemaVersion:1,updatedAt:firebase.firestore.FieldValue.serverTimestamp()},{merge:true});last=text;console.log('Fitness Master: cloud saved');}
    catch(e){console.error('Fitness Master Firestore write:',e);}
    finally{busy=false;}
  }
  ...
  window.FitnessMasterFirebaseSync={connected:true,version:1,pushNow:push,getStatus:()=>({uid:uid,ready:!!uid&&!!db})};
