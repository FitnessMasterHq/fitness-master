/* Fitness Master — Firestore sync v6: logical de-duplication + safe merge */
(function(){
  'use strict';
  ...
  function logicalKey(x,type){
    ...
    if(type==='logs')return [x.date||'',x.name||'',x.set||'',x.kg||'',x.reps||'',x.rir||''].join('|');
    if(type==='bodyLogs')return [x.date||x.dateISO||'',x.weight||'',x.fat||'',x.skeletalMuscle||'',x.waist||'',x.source||''].join('|');
    if(type==='activities')return [x.date||x.dateISO||'',x.type||x.name||'',x.duration||'',x.distance||'',x.hr||x.avgHR||''].join('|');
    return sig(x);
  }
  ...
  function subscribe(){if(unsubscribe)unsubscribe();unsubscribe=ref().onSnapshot(snap=>{if(!snap.exists)return;const remote=snap.data().state;if(!remote)return;const current=local();const merged=merge(current,remote);if(sig(merged)===sig(current)){last=sig(current);return;}localStorage.setItem(KEY,JSON.stringify(merged));last=sig(merged);status('Buluttan güncellendi ✓','ok');renderCurrent();window.dispatchEvent(new CustomEvent('fm-cloud-data-updated',{detail:{uid:uid}}));if(sig(merged)!==sig(remote))write(merged);},e=>{status('Bulut dinleme hatası','error');console.error('Fitness Master Firestore listener:',e);});}
  ...
  window.FitnessMasterFirebaseSync={connected:true,version:6,pushNow:push,getStatus:()=>({uid:uid,ready:!!uid&&!!db})};
